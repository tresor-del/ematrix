...
def tasks_view(request):
    ...
    pending_task_count = tasks.count() - completed_task_count

    # Définis les catégories et leurs propriétés
    categories = [
        {
            'title': "Do first",
            ...
            'border': "rgb(230, 70, 70)",
            'border2': "rgb(204, 118, 118)",
        },
        {
            'title': "Schedule",
            'desc': "Less urgent, but important",
            'priority': "important but not urgent",
            'class': "not-urgent-important",
            'color': "rgb(148, 206, 137)",
            'border': "rgb(165, 223, 99)",
            'border2': "rgb(148, 206, 137)",
        },
        {
            'title': "Delegate",
            'desc': "Urgent, but less important",
            'priority': "not important but urgent",
            'class': "urgent-not-important",
            'color': "rgba(98, 201, 233, 0.808)",
            'border': "rgba(98, 201, 233, 0.808)",
            'border2': "rgb(148, 190, 207)",
        },
        {
            'title': "Don't do",
            'desc': "Neither important nor urgent",
            'priority': "not important and not urgent",
            'class': "not-urgent-not-important",
            'color': "rgba(76, 79, 80, 0.808)",
            'border': "rgba(76, 79, 80, 0.808)",
            'border2': "rgb(47, 67, 75)",
        },
    ]

    # Ajoute les tâches filtrées à chaque catégorie
    for cat in categories:
        cat['tasks'] = tasks.filter(priority=cat['priority'])
...
@csrf_exempt
def delete_all_tasks(request, category):
    tasks = Task.objects.filter(author=request.user, priority=category)
