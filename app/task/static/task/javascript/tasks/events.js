...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.delete-all-tasks').forEach(button => {
        deleteAllTaskListener(button);
    });
    ...
    function deleteAllTaskListener(element) {
        element.addEventListener('click', function () {
            const desc = this.dataset.desc; // ou utilise data-priority si tu préfères
            // Sélectionne tous les .task-item du quadrant correspondant
                ...
                .then(response => {
                    if (response.ok) {
                        Swal.fire({
                            ...
                            text: "You have deleted all tasks for this quadrant",
                            icon: 'warning',
                            confirmButtonText: 'Got it!',
                            timer: 5000,
                            customClass: {
                        ...
                        // Supprime tous les éléments du DOM
                        taskItems.forEach(item => item.remove());
                    } else {
                        alert('Failed to delete the tasks.');
                    }
                })
                .catch(error => {
                    console.error('Error:', error);
